...
def _island_from_coordinates(clicked):
    ...
    # Marker clicks report the marker coordinates. Ignore ordinary map clicks.
...
def draw_island_map():
    ...
    # Use explicit OpenStreetMap tiles: no CARTO API key or environment variable.
    ...
    # Coordinate-based marker selection is independent of popup HTML/API changes.
    selected = _island_from_coordinates(result.get("last_object_clicked"))
    if selected:
        return selected

    # Compatibility fallback for streamlit-folium versions returning marker text.
